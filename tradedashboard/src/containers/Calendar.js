...
const Calendar = () => {
  ...
  const handleDateClick = (selectedDate) => {
    // You can also call your setDate function here if needed
    setDate(selectedDate);
  };

  const { sendMessage } = useWebSocket("ws://3.142.132.115:5000/th_backend/");

  const [startDate, setStartDate] = useState(
    new Date().toISOString().split("T")[0]
  );
  const endD = addDaysToDate(startDate, 6);
  const [endDate, setEndDate] = useState(endD);
  const [formattedDate, setFormattedDate] = useState({
    startDate: formatDateToYearMonthDate(new Date()),
    endDate: formatDateToYearMonthDate(addDaysToDate(new Date(), 6)),
  });
  ...
  const handleDateChage = (date) => {
    const newEndDate = addDaysToDate(date, 4);
    setStartDate(date);
    setEndDate(newEndDate);
    setFormattedDate({
      startDate: formatDateToYearMonthDate(date),
      endDate: formatDateToYearMonthDate(newEndDate),
    });
  };
  ...
  const generateWeekDays = () => {
    ...
    return days.map((i) => (
      <li className="nav-item" onClick={() => handleDateClick(i)}>
        <a
          className={` nav-link btn_main_hp ${startDate == i ? "active" : ""}`}
          id="pills-Monday-tab"
          data-toggle="pill"
          href="#pills-Monday"
          role="tab"
  ...
  const filterDataByBmo = (data) => {
    return {
      date: data.date,
      data: data.data?.filter((item) => item.time === "bmo"),
    };
  };
  ...
  return (
    ...
    <div   id="capture" >
      ...
      <div className=" container-fluid area-padding Calender_Section_hp">
        <div className="d-flex align-items-center mb-3 ">
          <ul
            className="nav nav-pills table-responsive Calender_list_hp mr-3"
            id="pills-tab"
            role="tablist"
          >
            {generateWeekDays()}
        ...
        <div
          className="tab-content Calender_listContent_hp mt-4"
          id="pills-tabContent"
        >
          <div
          ...
          >
            <div className="DailyLevel_shares_listContent_item">Hello</div>
          </div>
          ...
          >
            <div className="row">
              <div className="col-xl-4 col-lg-12 col-md-12 mb-4">
                <EarningSection startDate={date} endDate={formattedDate.endDate} />
              </div>

              <div className="col-xl-4 col-lg-12 col-md-12 mb-4">
                <EconomicSection startDate={date} endDate={formattedDate.endDate} />


              </div>

              <div className="col-xl-4 col-lg-12 col-md-12 mb-4">
                <IpoSection startDate={date} endDate={formattedDate.endDate} />

                <DividendSection startDate={date} endDate={formattedDate.endDate} />
