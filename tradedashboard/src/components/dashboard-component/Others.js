...
import { addOtherData, deleteOther, getOthersData, getOthersnewsData, postOthers } from "../../services/DashboardServices";
import NavBottom from "../nav/NavBottom/NavBottom";
import Side from "../sidebar/side";
import { otherFeedPosts } from "../../services/UserServices";
...
function Others() {
  ...
  const [othersnewsdata, setOthersnewsdata] = useState([]);
...
const handleNavigate = (ticker) =>{
  ...
  const getOtherdata = async () => {
    try {
      setLoading(true);
      const response = await getOthersData();
      if (response.status == 200) {
        const res = response?.data?.data;
        setOthersdata(res);
        setLoading(false);
        fetchData(res.map((item) => item?.symbol)); 
        getOthersnewsdata(res);
      }
    } catch (error) {
      setLoading(false);
  ...
  const getOthersnewsdata = async (res) => {
    try {
      setIsloading(true);
      const response = await getOthersnewsData(res.map((item) => item?.symbol));
      if (response.status == 200) {
        const res = response?.data?.data;
        setOthersnewsdata(res);
        setIsloading(false);
      }
    } catch (error) {
      setIsloading(false);
  ...
  const fetchData = async (jcoopdata) => {
  
    const limit = 15;

    try {
      setIsloadingFeed(true);
      const response = await otherFeedPosts(jcoopdata,limit, 1);
  ...
  const postOthersData = async (data) => {
    try {
      const response = await postOthers(data);
  ...
  const handleAddStock = async (data) => {
    ...
    try {
      const response = await addOtherData(Obj1);
      if (response?.status === 200) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          const response = await deleteOther(uid,symbol);
                      ...
                      <tbody>
                        {loading ? (
                          ...
                          othersdata?.map((item, index) => (
                            ...
                            >
                                            <td className="cursor-pointer" onClick={()=>handleNavigate(item?.symbol)} data-tooltip-id="my-tooltip-1" data-tooltip-content={item?.name}>{item.symbol}
                      ...
                      <div className="searching_Reault_Right_overview_table border-0 p-0 ">
                        ...
                        <div className="table-responsive tabalignn custom-table searching_Reault_Right_table_Responsive">
                          ...
                          <tbody>
                              ...
                              ) : othersnewsdata && othersnewsdata.length > 0 ? (
                                othersnewsdata.map((news, index) => (
                                  <tr key={index}>
                                    <td>
